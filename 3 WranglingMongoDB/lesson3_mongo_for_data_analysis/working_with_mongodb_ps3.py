    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            if line['binomialAuthority_label'] == 'NULL':
                ...
                new_key = line['rdf-schema#label']                
                new_value = re.sub(r'\(.*?\)', '', line['binomialAuthority_label'])
...
def update_db(data, db):
    ...
    """
    distinct_labels = db[COLLECTION_NAME].distinct('label')
    
    for label in distinct_labels:
        try:
            authority_value = data[label]
        except KeyError:
        ...
        db[COLLECTION_NAME].update_many(
            {"label": label},
            {"$set": {"classification.binomialAuthority": authority_value}}
        )
