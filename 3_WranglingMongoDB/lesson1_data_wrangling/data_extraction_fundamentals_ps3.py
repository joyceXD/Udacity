...
API_KEY = { "popular": "c9124346cb144daea8c79ebe9149ed0a",
            "article": "c9124346cb144daea8c79ebe9149ed0a"}
...
def article_overview(kind, period):
    ...
    for i in range(0, len(data)):
        dict_section_title = {data[i]['section'] : data[i]['title']}
        titles.append(dict_section_title)
        
        media = data[i]['media']
        
        for j in range(0, len(media)):
            metadata = media[j]['media-metadata']
            for k in range(0, len(metadata)):
                format = metadata[k]['format']
...
def article_overview_1(kind, period):
    data = get_from_file(kind, period)
    titles = []
    urls =[]
